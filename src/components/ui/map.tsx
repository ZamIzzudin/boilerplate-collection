...
function PopupCloseButton({ onClick }: Readonly<{ onClick: () => void }>) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function ControlGroup({ children }: Readonly<{ children: React.ReactNode }>) {
  return (
...
function CompassButton({ onClick }: Readonly<{ onClick: () => void }>) {
  const { map } = useMap();
